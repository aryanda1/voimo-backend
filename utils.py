...
def get_output_folder(output_path, batch_folder):
    out_path = os.path.join(output_path,time.strftime('%Y%m%d%H%M%S'))
    if batch_folder != "":
...
class SamplerCallback(object):
    ...
    def __init__(self, args, device,model,mask=None, init_latent=None, sigmas=None, sampler=None,
                  verbose=False):
...
def generate(args, device,model,frame = 0, return_latent=False, return_sample=False, return_c=False,):
